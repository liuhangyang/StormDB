/*************************************************************************
	> File Name: StormDB_conf.h
	> Author:yanglongfei 
	> Mail:yanglongfei@xiyoulinux.org 
	> Created Time: 2016年12月24日 星期六 13时00分26秒
 ************************************************************************/
...
class Config {  
    // Data  
protected:  
    std::string m_Delimiter;  //配置文件中key和value之间的分隔符. 
    std::string m_Comment;    //注释的提示符,比如#
    std::map<std::string,std::string> m_Contents;  //存储从配置文件提取的key和value;  
...
public:  
    ...
    Config();  
    template<class T> T Read( const std::string& in_key ) const;  //根据key查找value，找不到返回默认值.
...
/**
 *根据key找value，找到则返回,找不到返回默认值value(第二个参数)
 *
 */
template<class T>  
